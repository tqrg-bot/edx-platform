...
class CourseDateSummaryTest(SharedModuleStoreTestCase):
    ...
    def test_enabled_block_types_with_assignments(self):
        ...
        with patch('lms.djangoapps.courseware.courses.get_dates_for_course') as mock_get_dates:
            ...
            self.assertEqual(len(blocks), len(expected_blocks))
            self.assertEqual(set(type(b) for b in blocks), set(expected_blocks))
            assignment_blocks = filter(lambda b: type(b) == CourseAssignmentDate, blocks)
            for assignment in assignment_blocks:
                self.assertNotEqual(assignment.title, 'Third nearest assignment')
                self.assertNotEqual(assignment.title, 'Past due date')
                self.assertNotEqual(assignment.title, 'Not returned since we do not get non-graded subsections')
                if assignment.title == 'Released':
                    self.assertEqual(assignment.link_text, 'Go to assignment')
                elif assignment.title == 'Not released':
                    self.assertIsNone(assignment.link)
                    self.assertIsNone(assignment.link_text)
            ...
            for assignment in assignment_blocks:
                ...
                if assignment.title == 'Released':
                    ...
                    self.assertIsNone(assignment.link)
                    self.assertIsNone(assignment.link_text)
                elif assignment.title == 'Third nearest assignment':
                    # It's still not released
                    self.assertIsNone(assignment.link)
                    self.assertIsNone(assignment.link_text)
                elif assignment.title == 'Past due date':
                    self.assertTrue(now > assignment.date)
                    self.assertEqual(assignment.link_text, 'Go to assignment')
